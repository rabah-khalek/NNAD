 ...
 public:
  ...
  // Constructors
   FeedForwardNN(std::vector<int> const &Arch,
                 ...
                 bool const &LinearOutput = true) : _Arch(Arch),
                                                    _ActFun(ActFun),
                                                    _dActFun(dActFun),
   ...
   FeedForwardNN(std::vector<int> const &Arch,
       ...
       FeedForwardNN(Arch, 0, ActFun, dActFun, LinearOutput)
   {
     SetParameters(Pars);
   }
   ...
   void SetLink(int const &l, int const &i, int const &j, T const &Par) { _Links.at(l).SetElement(i, j, Par); }
   void SetBias(int const &l, int const &i, T const &Par) { _Biases.at(l).SetElement(i, 0, Par); }
   ...
   std::vector<T> Derive(std::vector<T> const &Input) const
   {
     ...
     y.insert({nl - 1, Matrix<T>{M, (_LinearOutput ? [](T const &x) -> T { return x; } : _ActFun)}});
     z.insert({nl - 1, Matrix<T>{M, (_LinearOutput ? [](T const &) -> T { return 1; } : _dActFun)}});
     ...
     // the unity matrix.
     Matrix<T> Sigma{_Arch[nl - 1], _Arch[nl - 1], std::vector<T>(_Arch[nl - 1] * _Arch[nl - 1], 0.)};
     for (int k = 0; k < _Arch[nl - 1]; k++)
       Sigma.SetElement(k, k, 1);
